...
contract AppleRewardPool is Ownable {
    ...
    function setApplePertime(
        uint256 _pid,
        uint256 _ApplePertime
    ...
    function getMultiplier(
        ...
        uint256 to = block.timestamp < pool.endtime
            ? block.timestamp
            : pool.endtime;
        if (from >= to) {
            return 0;
    ...
    function updatePool(uint256 _pid) public validatePool(_pid) {
        ...
        if (totalStake == 0) {
            pool.lastRewardtime = block.timestamp <= pool.endtime
                ? block.timestamp
                : pool.endtime;
            return;
        }
        ...
        pool.accApplePerShare = pool.accApplePerShare.add(
            AppleReward.mul(1e18).div(totalStake)
        );
        pool.lastRewardtime = block.timestamp < pool.endtime
            ? block.timestamp
            : pool.endtime;
    ...
    function pendingApple(
        ...
        if (block.timestamp > pool.lastRewardtime && totalStake > 0) {
            uint256 multiplier = getMultiplier(pool);
            uint256 AppleReward = multiplier.mul(pool.ApplePertime);
            accApplePerShare = accApplePerShare.add(
                AppleReward.mul(1e18).div(totalStake)
            );
        }
        return
            user
                .Applepending
                .add(user.amount.mul(accApplePerShare).div(1e18))
                .sub(user.ApplerewardDebt);
    }
    ...
    function rate1() public view returns (uint256) {
        uint256 _price;
        (uint256 _amount0, uint256 _amount1, ) = UniswapV2pair(pair2)
            .getReserves();
    ...
    function deposit(uint256 _pid, uint256 _amount) public validatePool(_pid) {
        ...
        updatePool(_pid);
        if (user.amount > 0) {
            uint256 Applepending = user
                .amount
                .mul(pool.accApplePerShare)
                .div(1e18)
                .sub(user.ApplerewardDebt);
    ...
    function reclaimAppleStakingReward(uint256 _pid) public validatePool(_pid) {
        ...
        UserInfo storage user = users[_pid][msg.sender];
        updatePool(_pid);
        uint256 Applepending = user.Applepending.add(
            user.amount.mul(pool.accApplePerShare).div(1e18).sub(
                user.ApplerewardDebt
            )
        );
        if (Applepending > 0) {
